...
const categories: { name: Category, subcategories?: (RopaSubcategory | HogarSubcategory)[] }[] = [
    { name: 'Ropa', subcategories: ['Bebés', 'Casual', 'Sport', 'Ejecutiva', 'Artesanal']},
    { name: 'Accesorios' },
    { name: 'Hogar', subcategories: ['Fundas', 'Cojines']},
    { name: 'Calzado' },
    { name: 'Carteras' },
];
...
export default function EditProductPage() {
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>, index: number) => {
    ...
    const folderPath = subcategory ? `${category.toLowerCase()}/${subcategory.toLowerCase()}` : category.toLowerCase();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || !description || !category || !price || !cost || !stock || !sizes || imageUrls.every(url => url === null)) {
        toast({
    ...
    if (selectedCategoryObject?.subcategories && !subcategory) {
        toast({ title: 'Subcategoría requerida', description: 'Por favor, selecciona una subcategoría.', variant: 'destructive'});
        return;
    }
              ...
              <Card>
                ...
                <CardContent className="grid gap-6">
                  <div className="grid gap-3">
                    ...
                    <Input id="name" type="text" className="w-full" required value={name} onChange={(e) => setName(e.target.value)} />
                  </div>
                  <div className="grid gap-3">
                    <Label htmlFor="description">Descripción</Label>
                    <Textarea id="description" className="min-h-32" required value={description} onChange={(e) => setDescription(e.target.value)} />
                  </div>
                </CardContent>
              </Card>
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Sube hasta 4 imágenes. Debes seleccionar una categoría primero.</CardDescription>
                </CardHeader>
                <CardContent>
              ...
              <Card>
                ...
                <CardContent className="grid gap-6">
                  <div className="grid gap-3">
                    ...
                    <Select required onValueChange={(v: Category) => { setCategory(v); setSubcategory(''); }} value={category}>
                  ...
                  {selectedCategoryObject?.subcategories && (
                    <div className="grid gap-3">
                        ...
                        <Select required onValueChange={(v) => setSubcategory(v)} value={subcategory}>
                   ...
                   <div className="grid gap-3">
                    ...
                    <Input id="sizes" type="text" placeholder="S, M, L, XL" required value={sizes} onChange={(e) => setSizes(e.target.value)} />
                    <p className="text-xs text-muted-foreground">Separar por comas. Ej: S, M, L, Única</p>
                  </div>
